  ...
  return (
    <div className="flex w-full min-w-[380px] max-w-[560px] items-center justify-center p-10 lg:w-1/2">
      <div className="w-full max-w-[360px]">
        ...
        <h1 className="text-[26px] font-extrabold tracking-tight">Добро пожаловать</h1>
        <p className="mb-7 mt-1.5 text-[13px] text-muted-foreground">Войдите в свой аккаунт</p>

        <form onSubmit={f.handleSubmit} className="space-y-4">
          <div className="space-y-1.5">
            <Label htmlFor="login-username" className="text-xs text-muted-foreground">Логин</Label>
            <Input id="login-username" type="text" placeholder="your_username" autoComplete="username" autoFocus
              value={f.username} onChange={(e) => f.setUsername(e.target.value)} disabled={f.isLoading} />
          </div>

          <div className="space-y-1.5">
            <Label htmlFor="login-password" className="text-xs text-muted-foreground">Пароль</Label>
            <div className="relative">
              <Input id="login-password" type={f.showPassword ? "text" : "password"}
          ...
          <Button type="submit" disabled={!f.canSubmit}
            ...
            {f.isLoading ? (<><Loader2 className="mr-2 h-4 w-4 animate-spin" />Входим...</>) : "Войти"}
          </Button>
