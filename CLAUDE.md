...
В начале **каждого** промежуточного ответа — первой строкой — пиши:

> Сейчас: [что делаем, простым русским языком, максимум 10 слов]

Примеры: `Сейчас: настраиваем систему зон ответственности`, `Сейчас: чиним баг в оплате`, `Сейчас: пишем документацию по подпискам`.
Без терминов, без английского, без названий классов и функций — только суть.

### Финальный отчёт

Когда выдаёшь **итоговый результат** работы (задача выполнена, список багов пофикшен, фича реализована) — начинай с блока-напоминания:

> **Сессия:** [кратко — тема сессии, например «исправление багов шеринга»]
> **Задача:** [что именно поручил пользователь, 1–2 предложения]
> **Результат:** [что сделано, кратко]

Этот блок нужен, чтобы пользователь, переключаясь между несколькими параллельными сессиями, сразу понимал: куда попал, что поручал, что получил. Писать его **только** в финальном отчёте — не в промежуточных размышлениях, не в общении с суб-агентами.
...
│   ├── onboarding/     → Достижения, welcome-модалка, прогресс (TIER 3)
...
При получении задачи — определи домен и прочитай `docs/systems/X.md` перед тем как трогать код.
**Перед началом работы** — прочитай `docs/systems/README.md` (правила, содержание, обязательные процедуры).
При **значимых изменениях** в домене — обновить соответствующий `docs/systems/X.md`. Значимые: новая модель, новый API-эндпоинт, изменение бизнес-правила, перенос файла. Незначимые: фикс бага, рефактор без смены интерфейса.
При **кросс-доменном баге** — читать оба файла + секцию "Интеграции" в каждом.

### Добавление фич — обязательный чеклист

При реализации **любой новой фичи** агент обязан самостоятельно, без напоминаний:

1. **Документация:** создать или актуализировать спеку в `docs/systems/features/<feature>.md`.
2. **Feature gating:** увязать фичу с системой подписок — `FeatureFlag` на бэкенде, `useFeatureGate()` на фронтенде, backend enforcement. Подробности в `docs/systems/subscriptions.md`.
3. **Обновить README:** добавить фичу в таблицу `docs/systems/README.md` → секция «Фичи».
4. **Обновить домен:** если фича затрагивает существующую подсистему — актуализировать `docs/systems/X.md`.

Фича без гейтинга подписок и без документации — это баг. Не пропускай эти шаги.

| Если проблема в... | Домен | Читать |
|--------------------|-------|--------|
| Авторизация, JWT, login by email, AuthGuard | `auth` | `docs/systems/auth.md` ✓ |
| Элементы, workspace, lightbox, bulk actions, DnD | `elements` | `docs/systems/elements.md` ⚠ |
| AI-генерация, провайдеры, Celery, polling | `generation` | `docs/systems/generation.md` ⚠ |
| Кредиты, транзакции, цены, дебет/рефунд | `credits` | `docs/systems/credits.md` ⚠ |
| Тарифы, подписки, feature gating, лимиты | `subscriptions` | `docs/systems/subscriptions.md` ✓ |
| Проекты, группы, сцены, сортировка | `projects` | `docs/systems/projects.md` ⚠ |
| Шеринг, комментарии, реакции, ревью | `sharing` | `docs/systems/sharing.md` ⚠ |
| Кабинет, аналитика, уведомления, настройки | `cabinet` | `docs/systems/cabinet.md` ✓ |
| Онбординг, достижения, welcome, награды | `onboarding` | `docs/systems/onboarding.md` ✓ |
| Транзакционные письма, SMTP, SPF/DKIM/DMARC | `email` | `docs/systems/email.md` ✓ |
| S3, загрузки, presigned URLs, thumbnails | `storage` | `docs/systems/storage.md` ⚠ |
| Workspace: грид, карточки, лайтбокс, генерация UI | `workspace` | `docs/systems/workspace.md` ✓ |
| WebSocket, Celery-конфиг, Redis, Docker, Nginx | `infra` | `docs/systems/infra.md` ⚠ |

⚠ — файл ещё не написан, использовать `docs/MODULE_MAP.md` как fallback.
✓ — файл актуален.
...
- **Не использовать иконку `Sparkles` (lucide-react).** Запрещена на уровне проекта.
